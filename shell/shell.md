...
```
set -o pipefail     # pipefail を有効にする
set +o pipefail     # pipefail を無効にする
```
...
パイプラインの終了ステータスは、通常(pipefail無効)は、最後のコマンドの終了ステータスになる。

- cmd1 や cmd2 でエラーが発生しても cmd3 の終了ステータスになる。

pipefail を有効にすると、エラーになった最後のコマンドの終了ステータスになる。

- cmd1 と cmd2 でエラーになった場合、cmd2 の終了ステータスになる。
- 「最後の」というのは時刻的なタイミングでははく、パイプラインの後にある方ということ。
- この場合でも cmd3 は実行はされる。あくまで終了ステータスがどうなるかのはなし。


SIGPIPE まわりの注意点

下記のような、パイプの下流の方が先に終了してしまう場合、
上流のコマンドがcloseされたパイプに書き込もうとして SIGPIPE で終了する。

```
seq 100000 | head -n 10
```

pipefail 無効の場合は、

- 標準出力に10行分の結果が得られる。
- パイプライン全体の終了ステータスは、最後のコマンドの結果になるので、「成功」。

pipefail 有効の場合は、

- 標準出力に10行分の結果が得られる。
- パイプライン全体の終了ステータスは、SIGPIPEしたseqコマンドの終了ステータス。つまり「失敗」。 
- これは特に `set -e` している場合に、そこで終了してしまって困る。

これの回避としては

- 一時的に pipefail をオフにする。(そうするとエラーを検知できないケースが出てくるデメリットはある)
- `|| true` などして必ず成功するようにする。(そうするとエラーを検知できない...以下略)
- 下記のようなSIGPIPEを無視するラッパー関数を書く
...
出典: [シェルスクリプトで SIGPIPE を無視する方法 #Bash - Qiita](https://qiita.com/ko1nksm/items/236f1b941626ed651073)
...
```
                                <<'EOS'  <<"EOS"  <<EOS
変数展開      $var ${var}       しない   しない   する
コマンド置換  $(...) `...`      しない   しない   する
算術展開      $((...))          しない   しない   する
バックスラッシュ特別解釈        しない   しない   する
```
...
{ }
  - 変数展開のとき、必要があれば付ける
  - 一連のコマンドをカレントシェルで実行する。 (中のコマンドの最後には ; が必要)
...
`"$*"`
  ...
  - すべての位置パラメータを結合して、一つの引数として結合にする時などに使う。IFSの1つ目を挟んで結合する。
  - 他の言語で例えるなら、配列を結合 (join) して一つの文字列を作る処理に相当する

`"$@"`
  `"$1" "$2" "$3"` みたいな感じ

  - すべての位置パラメータを別のコマンドにそのまま渡す場合
  - for ループで繰り返すとき


`$*` と `$@` の違いは、複数の引数を1つに結合するかしないかの違い。
通常は複数の引数を一つに結合したりしないので `"$@"` を使いましょう。
またダブルクォートなしの `$*` や `$@` はバグに近いレベルの間違った使い方なので覚えなくてよい。
...
- シェルスクリプトの $* と $@ の違いと雑学色々 #Bash - Qiita
https://qiita.com/ko1nksm/items/98952723d7a71799673c
- シェルスクリプトの変数はダブルクォートしなければいけない！という話 #Bash - Qiita
